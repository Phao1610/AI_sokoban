...
class Game:
    ...
    def process_event(self, event):
        ...
        if event.type == KEYDOWN:
            ...
            if event.key in [K_UP, K_DOWN, K_LEFT, K_RIGHT, K_z, K_s, K_q, K_d]:
                ...
                if self.has_win():
                    self.index_level += 1
                    if (self.index_level == 17): # Replace 19 to 17
                        self.index_level = 1
                    self.scores.save()
                    self.load_level()
            if event.key == K_r:
    ...
    def auto_move(self):
        #Xuất ra level đang chạy
        print('Level {}'.format(self.index_level))
        #chạy level đó bằng thuật toán DFS
        strategy = get_move(self.level.structure[:-1], self.level.position_player, 'dfs')
        #chạy level đó bằng thuật toán BFS 
        # strategy = get_move(self.level.structure[:-1], self.level.position_player, 'bfs')
        #chạy level đó bằng thuật toán UCS 
        # strategy = get_move(self.level.structure[:-1], self.level.position_player, 'ucs')


        # xuất ra những file txt tương ứng với level
        with open("assets/DFS/Solverlevel_" + str(self.index_level) + ".txt", 'a') as solver_file:
            #xuất tên thuật toán đang chạy vào file
            solver_file.write('[Sequence Action ' + strategy[2] + ']')
            #xuất trang thái vừa được thực hiện
            solver_file.write('%s, \n' % strategy[0])
            #kiểm tra số trang ra màn hình
            print(len(strategy[0]))
            #xuất số trangj thái vào file txt 
            solver_file.write('%s \n' % len(strategy[0]) )
            #Xuất thời gian thực hiện vào file txt
            solver_file.write('[Time action ' + strategy[2] + '] %s,'  % strategy[1])
            solver_file.write('\n')
